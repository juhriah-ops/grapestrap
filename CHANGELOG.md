...
### Added
- **Template sections in the Library**: the two bundled starters' best page sections are now available one click at a time, without starting a whole project from that template. The Library panel gains a **Graphite** group (hero carousel, spotlight split, hexagon feature row, three-column post-card grid, reach list, contact form, slim footer) and an **Orbit** group (hero banner, accent band, ring-icon feature grid, avatar row, sidebar layout, two-rail layout, full-width article, five-column footer) — each row with a wireframe preview and a one-line description. Inserting one drops a **free, fully editable copy**: nothing is locked, nothing is linked back, and you can take it apart on the page. Sections land as siblings, so clicking insert twice gives you two page bands, not one nested in the other.
