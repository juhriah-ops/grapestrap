...
### Added
- **Bootstrap is now an editable stylesheet, not a locked vendor file**: a fourth tab, **Bootstrap**, joins DOM / Properties / Custom CSS on the right, holding the project's own `site/assets/css/bootstrap.css`. Change a variable, retune a component, delete a chunk you never use — the canvas repaints as you type (the preview reads your unsaved buffer, nothing is written until you save), Ctrl+S writes it back to the project, and **export ships your version**, never a fresh copy of the shipped one. Every project keeps its own copy, so editing one project's Bootstrap never touches another's. Projects built on a starter that brings its own framework (Graphite, Orbit) show a short note pointing at Site Files instead — there is no app-managed Bootstrap in those to edit. The tab hides and restores from **View → Toggle Bootstrap CSS** like the others, and saved workspace layouts made before this existed gain the tab the next time they're applied. One caveat worth knowing: unsaved Bootstrap edits are deliberately left out of the 30-second crash snapshot (a 280KB stylesheet on every tick, for a file that changes rarely), so they are not crash-recovered — same as file-tab edits.
- **A heads-up when a bundled section was built for a different Bootstrap.** Every project now remembers which Bootstrap it was created or imported with (shown to Bootstrap 5.3-era projects made before this existed as "legacy," so the check still has something to compare against). Inserting a bundled Library section or one of your own library items whose Bootstrap has drifted a MAJOR version away from your project's now shows a quick "Insert Anyway / Cancel" — Cancel backs out cleanly with nothing added (no markup, no CSS, no runtime files), Insert Anyway goes ahead exactly as before. Minor/patch drift never asks, and projects on a starter's own vendored framework (Graphite, Orbit) never see it at all — there's nothing of the app's own Bootstrap to compare against in those.
- **Right-click a style rule and go to where it is written.** In the Style Manager's **Cascade** list, every rule now knows which stylesheet produced it, and its right-click menu takes you there: a rule from your own stylesheet opens the **Custom CSS** tab, a Bootstrap rule opens the **Bootstrap** tab, and in both cases the selector itself is selected in the editor and scrolled into view — hidden panels un-hide and the tab comes forward on the way. A rule from any other stylesheet inside the project (a starter's vendored sheets, for instance) opens that file in the code view instead; a rule from somewhere we cannot open — a CDN, a `<style>` tag in the page — says so plainly rather than offering a menu item that goes nowhere. The same two "Go to Rule" entries are on the right-click menu of every **class chip** in the Properties panel, greyed out for the stylesheet that has no rule for that class, so the menu tells you where a class is defined before you go looking.
- **Class-name suggestions on the add-class input.** Start typing in the Properties panel's class chip input and a popover lists matches from four places at once: every utility Bootstrap ships (`col-6`, `mt-3`, `bg-primary-subtle`, …), whatever your project's own edited Bootstrap sheet actually defines, your project's own stylesheet, and classes already in use elsewhere on the page — each hinted with its Bootstrap docs topic where one applies, or marked "project class" otherwise. Arrow keys move the highlight (wrapping, and opening the popover if it wasn't already), Enter or a click picks the highlighted row, Tab picks it and moves on, and Esc dismisses the popover without touching what you typed. Nothing about typing a brand-new class name changes: with nothing highlighted, Enter commits it exactly as before — the popover only ever opts in, never in the way.
- **An Attributes section in the Properties panel**, which changes with what you have selected. A link gets href, target and rel; a Bootstrap button-link also gets its label; an image gets src and alt; a heading gets a level dropdown that retags it in place and keeps its classes; a list gets bulleted/numbered plus Add item and Remove item; an iframe gets its src. Clearing a field removes the attribute rather than leaving `href=""` behind in the saved page, and elements locked by a master template show their values but cannot be edited. The bottom property strip draws on the same set, so it picked up rel and button labels for free — everything it showed before, it still shows.
- **Table editing without touching the code view**: select a table and the Attributes section offers a header-row toggle, striped / bordered / hover, and buttons to add or remove a row or column. The same four operations sit on the right-click menu anywhere inside a table — used from a cell they act next to *that* cell, so "Add Column" on the second column inserts a third one beside it rather than at the far right. Adding a column widens the header row too, and the last row and last column refuse to be removed. Each operation is a single undo step, however many cells it touched.
- **Edit Link…** in the right-click menu for any link: href, target and rel in one dialog. Applying all three counts as one undo step, so Ctrl+Z takes the link back to exactly what it was instead of unwinding one attribute at a time.
- **Select Parent / Select Child** in the element right-click menu (canvas and DOM tree): jump straight to a component's parent — or one of its children — without hunting through the DOM tree panel. Select Parent is disabled at the wrapper/body root; Select Child lists up to 6 children by tag and first class (e.g. `div.row`), with a disabled "… N more" row when there are more.
- **Reveal in Code View** and **Go to rule** in the element right-click menu (canvas and DOM tree). Reveal in Code View jumps to the selected element's markup in the code pane — switching into Split view first if you were in Design — and selects the whole block so you can see exactly where it lives. Go to rule lists up to 6 of the element's classes and jumps straight to that class's rule in the project's CSS, with a toast if no matching rule exists yet.
- **Template sections in the Library**: the two bundled starters' best page sections are now available one click at a time, without starting a whole project from that template. The Library panel gains a **Graphite** group (hero carousel, spotlight split, hexagon feature row, three-column post-card grid, reach list, contact form, slim footer, **navbar**) and an **Orbit** group (hero banner, accent band, ring-icon feature grid, avatar row, sidebar layout, two-rail layout, full-width article, five-column footer, **navbar**) — each row with a wireframe preview and a one-line description. Inserting one drops a **free, fully editable copy**: nothing is locked, nothing is linked back, and you can take it apart on the page. Sections land as siblings, so clicking insert twice gives you two page bands, not one nested in the other.
- **Navbar sections**: Graphite's navbar (top nav, two-level dropdown, off-canvas mobile panel) and Orbit's navbar (top nav, two-level dropdown, collapsing to Bootstrap's own mobile panel) join their template's Library group. Both are wired to the behaviors runtime on insert — the nested "Submenu"/"Layouts" item opens as a proper flyout on click (with Graphite's small arrow glyph), and the mobile toggler closes itself once a real link is chosen. Starter-only chrome is left out: Graphite's overlay-over-a-hero navbar skin and Orbit's seven-swatch theme picker are page/starter machinery, not something a bundled section should assume or carry along. Recreate Graphite's "floats over the hero" look yourself with `sticky-top` plus the Navbar panel's scroll effects.
- **Bundled sections in the Library panel (plumbing)**: the Library panel can now list read-only groups of shipped, pre-formatted sections alongside your own library items, and insert one as a free editable copy — its stylesheet rules appended to the project's Custom CSS under a marker comment (re-inserting never duplicates them, and your edits under a marker are never overwritten) and any images it needs copied into the project's assets. Insert → Library also stops dead-ending on an empty Insert tab and focuses the Library panel instead.
- **An Animation panel in the Style Manager**, for any element you select. The Style accordion gains an **Animation** section in three groups. **Reveal**: seven entrance effects (fade, fade up/down/left/right, zoom in/out) that play as the element scrolls into view — or on page load — with sliders for duration and delay and an **Animate once** switch for whether it replays every time the element comes back. Picking an effect plays it once on the canvas straight away, and a **Preview** button replays it whenever you want; the replay is a rehearsal only — nothing about it is saved, and it never lands on the undo stack. **On hover**: grow, shrink, lift, glow, or an underline sweep — pure CSS, so you can just hover the element on the canvas to see it. **Looping**: pulse, bounce, spin, float, and a marquee in either direction, each at slow, normal or fast. Everything lands in the markup you can read, as `data-` attributes on the element itself, and a setting that only restates what the animation already does by default is left out of the saved page entirely. **Remove all animation** strips the lot in a single undo step, each other change is one undo step of its own, and an element locked by a master template shows its settings read-only.
- **A Navbar panel in the Style Manager**, for the settings a navbar needs and no other element does. Select a navbar — or the header band around it — and the Style accordion gains a **Navbar** section: **Position** (static / sticky / fixed, written as Bootstrap's own `sticky-top` / `fixed-top` classes, with a reminder to pad the content under a fixed one), **Background on scroll** (become solid, or swap between a top colour and a scrolled colour you pick), a **Threshold** in pixels for how far the page scrolls first, **Shrink on scroll**, **Hide on scroll down**, and **Auto-close mobile menu** — whose dropdown marks which mechanism this navbar's own toggler uses, so you are not guessing between collapse and offcanvas. Everything lands in the markup you can read: Bootstrap classes and `data-` attributes on the `<nav>` itself, with the two swap colours as real rules in the project stylesheet. Switching a setting off removes its attribute rather than leaving a disabled leftover behind, each change is a single undo step, and the behaviors script is added to the project the first time you switch something on. Hide on scroll is greyed out (and says why) while the navbar is static, and a navbar locked by a master template shows its settings read-only. Selecting anything that is not part of a navbar shows a one-line note instead of controls.
- **Behaviors runtime (plumbing)**: projects can now carry a small, versioned behaviors script and stylesheet in their own `assets/`, driving scroll reveals, hover and looping effects, navbar scroll states, second-level dropdown menus and mobile-menu auto-close — all configured through `data-` attributes on the markup itself, so what the editor shows and what a visitor's browser does come from the same authored source. It switches on for a project the first time something needs it, the files land in the project (and ship with an export) rather than loading from anywhere else, and both respect the operating system's "reduce motion" setting. The two navbar sections, the Navbar panel and the Animation panel above are what use it today.
- **Orbit starter template**: the New Project dialog gains **Orbit** (blue) — a second complete Bootstrap 5 starter alongside Graphite, 5 pages (a self-describing homepage component tour plus left/right/dual-sidebar and full-width layout-tour pages) with its own vendored Bootstrap 5.3.8, Font Awesome 7.3.1, Roboto webfonts, theme stylesheet, and an in-page 7-theme switcher.
- **A real colour selector, not just a swatch grid**: the colour picker now opens on a **saturation/brightness spectrum** with a draggable thumb, a **hue strip** under it, and **R / G / B numeric fields** beside the hex box. Everything is bidirectional — drag the spectrum and the hex and RGB fields follow; type into any field and the thumbs move to match — and every one of those interactions writes the CSS rule live, so the canvas repaints as you drag instead of waiting for a commit. The Bootstrap palette, recents, eyedropper and Clear are unchanged and still one-shot (pick and the popover closes); the spectrum, hue strip and numeric fields keep the popover open so you can hunt for a colour. Values the picker cannot resolve to numbers — `var(--bs-primary)`, named colours, `transparent` — still work as before: the hex box and preview take them verbatim, the numeric fields blank out rather than showing a made-up 0/0/0, and the spectrum holds its last position.
- **Any colour, not just the Bootstrap eight**: the Background, Text and Border sub-panels each gain a **Custom** row — a chip that opens the full colour picker (hex, rgb, `var(--bs-*)`, eyedropper, recents) and stores the result as a real `background-color` / `color` / `border-color` declaration on the element's rule in the project stylesheet. No inline styles: the value lands in `style.css` scoped to the element's own class or id, so it stays hand-editable and survives export. Predetermined and custom are mutually exclusive — choosing a free colour drops the `bg-*` / `text-*` / `border-*` class, and clicking a theme swatch removes the free value — so the cascade never gets to pick a winner the user didn't. Elements with no custom class or id show the same "add a class first" hint the background-image row uses.
- **Opacity in the Display sub-panel**: the five Bootstrap steps (`opacity-0` … `opacity-100`) as pills, plus a **slider with a live % readout** for everything in between, which writes `opacity` onto the element's rule. The two surfaces evict each other the same way the colour rows do. The slider commits on release rather than on every pixel of travel, and seeds itself from the rule, then the active pill, then 100%.
- **Background-attachment** in the Background sub-panel's image controls: a fourth row (`scroll` / `fixed` / `local`) alongside Size, Position and Repeat, writing `background-attachment` onto the element's rule. Re-picking an image now carries over a chosen attachment the same way it already does for size, position and repeat.

### Fixed
- **Undo in Split view** (nola1 report: "we need to improve on the undo function, most issues are on the splitview screen"). Four separate faults, all of which only bit when the canvas and the code pane were on screen together:
  - **Just *looking* at the code pane threw away your canvas undo history.** Split view rebuilt the canvas from the code every single time the code pane lost focus, and that rebuild clears the canvas undo stack by design — so clicking into the code to read the markup and clicking back out silently destroyed every undo step you had (measured: a two-step stack went straight to zero, and Ctrl+Z afterwards did nothing at all). The rebuild now only runs when the code actually changed.
  - **Every canvas edit wiped the code pane's undo history** and threw the caret back to line 1. The design→code sync replaced the whole document on each edit, and Monaco discards the model's undo stack, cursor and scroll position when you do that. It now applies the smallest edit that produces the same text, so your own typing stays undoable, and the caret, scroll position and code folding survive a canvas edit.
  - **Ctrl+Z did nothing if the caret happened to be in the code pane.** Undo routed purely by focus, so dragging a block onto the canvas, clicking into the code to look at it, then pressing Ctrl+Z hit the code pane's empty history and dead-ended instead of undoing the block. Split view now routes by the most recent edit *you* made, and falls through to whichever pane still has something to undo. Redo mirrors the undo it reverses, so a sequence that crosses panes replays in the same order it was unwound. Pure Design and pure Code views are unchanged — they still route by focus.
  - **Undo in the code pane could unwind the sync's own rewrites**, reverting the code text while the canvas kept the change — a split that could not be recovered from. A code-pane undo now stops at the last synced state and hands over to the canvas instead of walking back through generated rewrites.
- **The colour picker could open partly off the bottom of the window** when its trigger sat low in a scrolled panel — the popover flipped above the trigger, but a trigger scrolled out of view reports a position below the viewport, so "above" was still off-screen and the palette and buttons were unreachable. It is now clamped to the viewport in both directions and scrolls internally if the window is shorter than the popover.
- **Picking a background image wiped a custom background colour** on the same element: the background-image row stripped every `background-*` declaration from the rule before writing, rather than just the four it owns.
- **Right-click on the canvas works again** (nola1 report: no Select Parent / Select Child in the menu). Two faults stacked. The canvas right-click was never wired up at all in most sessions — the setup step that installs it ran before the canvas frame was ready and quietly did nothing, so right-clicking the page gave you nothing (the same step also injects the project's stylesheet links, which were being restored a moment later by the project-open path, which is what kept the failure invisible). And when it did run, it worked out *which* element you right-clicked by reading the selection a moment after the click — which meant the menu was built for whatever had been selected **before**, or for nothing at all on a freshly opened page. That is what dropped Select Parent, Select Child and the Go-to-rule entries out of the menu. Right-click now selects the element under the pointer and menus that element, exactly as before in intent; right-clicking inside text you are editing no longer breaks out of the text.
- **Resize handles never appeared on containers** — sections, rows, cards, plain divs. Only text-ish elements got the drag-to-resize margin/padding edges and the column width grip, because the check for "is this locked by a master template?" was reading a flag that every structural element carries by default. Containers now get their handles; genuinely locked template chrome and library items still get none.
- **The navbar's "Top" swap colour had no visible effect** while the "Scrolled" one worked (nola1 report). The resting colour was written to the navbar's plain class, which any two-class state rule in the theme outranks — the Graphite starter's `.site-navbar.is-overlay { background: transparent }`, for one, so the bar never changed while the page sat at the top. Both halves of the swap are now written at the same weight, as `:not(.gs-nav-scrolled)` and `.gs-nav-scrolled` on the class you choose, which is also just the CSS a colour swap wants: one colour while the navbar is not scrolled, another while it is. (A Top colour set before this update stays where it was written; set it again to move it onto the new rule.)
- **Custom values could be written into a rule the cascade then overruled**, which read as the panel doing nothing and left you editing the stylesheet by hand. Two placements caused it, both fixed for every free value the Style Manager writes — the Custom colour rows, opacity, background images, and the navbar's swap colours. Where a stylesheet carried the same selector twice, the value went into the first rule and the second one won; the value now goes to the rule that actually wins. And where the selector belonged to a bundled Library section, the value was merged into the section's own fenced rules — those are left exactly as the section shipped them now, and your value is written as its own rule after them, which is both where it wins and where you can find it. Clearing a value hands the element back to the section's default rather than gutting the section.
- **Edit Tag and Wrap with Tag were disabled for every container element** (divs, sections, tables, and the like), and **Edit Link… for any `<a>` that doesn't default to inline-editable** — the same "editable defaults false on structural elements" misread already fixed this round for resize handles. Both now check the same lock the rest of the right-click menu uses, so they're enabled on ordinary containers and still correctly disabled on template-locked chrome.
- **A DOM tree row's indent was a hardcoded inline style**, not a stylesheet rule — a hygiene fix with no visible behavior change. Indent is now written as a CSS custom property on the row and read by a rule in the DOM tree's stylesheet.

### Changed
- **The Custom colour rows now show — and let you choose — the class they write to.** Under the colour chip in Background, Text and Border (and beside the navbar's swap colours) there is an **Applies to** line naming the exact selector the value lands on, as a dropdown of the element's other classes. On a bundled section, where an element can wear a page-wide class, its template's class and its own, the row used to silently take the first one — so setting one band's background repainted every section on the page with nothing on screen to say why. The default is unchanged, so nothing moves unless you aim it somewhere else; Bootstrap utility classes are never offered (a rule on `.mt-3` is nobody's intent), and the choice is remembered per element and per property for as long as the window is open. Elements with no class of their own still get the "add a class first" hint. In the Background panel the image controls follow the same target as the colour, so a background image and a background colour can't drift onto different classes.
...
- **Style Manager rule writers are selector-anchored**: the Background panel and pseudo-class state bar matched a target class against the *tail* of longer selectors, so styling an element whose class ends a theme's compound rule (e.g. `.carousel-item` vs Graphite's `.hero-carousel .carousel-item`) silently rewrote the theme rule in place — reformatting it and replacing its `background-*` declarations. Rules now only match when the entire selector is the target; compound rules stay byte-identical and the panel appends its own whole-selector rule instead. Bare-rule surgery moved from `background.js` into `css-rule-utils.js` under unit test (`tests/unit/css-rules.test.js`). Note: a rule appended this way can still lose the cascade to a more specific theme rule — that's ordinary CSS specificity, visible in the canvas either way.
...
### Fixed
- **Background images broke on export / Preview-in-Browser** (post-rc.2). The project stylesheet ships at `assets/css/style.css`, but its relative `url()`s were written site-root-relative (`assets/images/…`) — correct in the canvas (whose `<base>` is the site root), broken everywhere the stylesheet is loaded via `<link>`. Canonical convention is now FILE-RELATIVE to the stylesheet (`../images/…`): the BG picker writes that shape, the canvas rewrites urls to document-relative in memory at inject time (`src/shared/css-urls.js` — authored CSS is never modified; absolute/protocol/`data:`/`#fragment` urls and the legacy `assets/…` shape pass through), and export ships `style.css` byte-identical. Opening a project one-shot migrates exactly the legacy app-written shapes (`url("assets/(images|fonts|videos)/…")` → `url("../…")`) in memory and marks the CSS dirty so the next save persists it. Export (and therefore Preview-in-Browser) now copies the whole `site/` tree — excluding `pages/`, `templates/`, `library/`, `*.tmp` — instead of only `site/assets/`, so imported projects with files elsewhere under `site/` keep them; the in-memory CSS buffer is written after the copy and wins. New unit harness `npm run test:unit` (node:test) covers the rewrite shapes; e2e extended for canvas resolution, export byte-identity + whole-tree copy, and a preview-server bg-image probe.
...
### Fixed
- **Sidebars force-grew on splitter drag in a windowed layout** (reported nola1 2026-07-06: "sidebar windows both left and right side dont resize correctly when window not in max view. it only gets larger"). GL v2's splitter drag bounds SUM the `minWidth` of every tab in a stack (`onSplitterDragStart` → `calculateContentItemsTotalMinSize`) even though tabs display one at a time — our 3-tab side stacks got a 540px effective floor. In any window where a sidebar sat below 540px, every drag (either direction) was clamped to a positive offset: the sidebar jumped OUT to 540px and drag-stop persisted the bigger percentage. This is also the real mechanism behind the alpha.9 "splitter snaps ~50px then sticks" report (same clamp, smaller jump at maximized widths). Fix: per-tab `minWidth`/`minHeight` in `golden-layout-config.js` are now the intended per-stack floor ÷ tab count (60px × 3 = 180px effective). New spec drives real mouse drags on the splitters and asserts shrink-by-drag, floor clamp at ~180px, and symmetric grow.
- **Properties↔Custom CSS sync (the alpha.12 known follow-up).** The alpha.12 note mis-diagnosed this as two CSS scopes; both views actually write the SAME `projectState.current.globalCSS`. Real bug: the Custom CSS Monaco buffer only re-read state on `project:opened`, never on `project:css-changed` — so after a Style Manager write (background image, position, attachment, pseudo-class rules) the buffer was stale, and the next Custom CSS keystroke wrote the old text back over the Properties rule. Last-writer-wins, one direction only, which is why "it's not clear which window overrides which." `custom-css/index.js` now listens for `project:css-changed` and refreshes the buffer when it differs from state: a value-equality check makes the panel's own debounced emit a no-op (no refresh loop), a `refreshingFromState` flag keeps the programmatic `setValue` out of the change handler (no spurious re-emit/dirty), and the cursor is restored across the refresh. `getCssEditor()` is exposed on `window.__gstrap` as the spec handle.

### Changed (hygiene sweep 2026-07-06)
- **Electron 31 (EOL) → 43**, **@playwright/test 1.45 → 1.61**, **electron-builder 24 → 26**. The Electron bump required a `gstrap-plugin://` protocol change: Chromium 150 hard-blocks cross-origin module `import()` from the file:// renderer into schemes that don't opt into CORS, which silently killed all 5 plugin activations (app boots to an empty shell). The scheme now registers `corsEnabled: true` and the handler sends `Access-Control-Allow-Origin: *` (file:// documents send `Origin: null`, so the wildcard is the only workable grant; the scheme serves path-traversal-guarded plugin files only). `npm audit` prod-dependency advisories cleared: fast-uri, tmp, @tootallnate/once via `audit fix`; underscore (grapesjs → backbone transitive) pinned to 1.13.8 via `overrides`. Remaining: the vite-5/esbuild dev-server advisory — dev-tooling only, needs a vite 8 + vite-plugin-electron compat pass, deferred.
- **`npm run lint` works for the first time** — ESLint 9 flat config added (`eslint.config.js`: `eslint:recommended`, per-context globals for main/renderer/plugins/tests). All dead code it caught removed: unused imports (`getProjectRoot`, `dispose`, `getCanvasHtml`, `pageState`, `bgGradientPattern`, `displayPattern`, `toggleClass`), unused locals (`target`, `LEAF_TAGS`, two vestigial spec vars), and the never-called `pickNewProjectPath()` in ipc-handlers. Lint script now covers `tests/` too.
- `canvas/index.js` view-mode relayout now imports `relayoutAllMonaco()` instead of poking Monaco through the `window.__gstrap` global.
- `getCssEditor()` (custom-css panel) is no longer a dead export — it's the `window.__gstrap` spec handle.

### Tests
55/55 green on Electron 43. New specs: `Properties→Custom CSS sync: external globalCSS write refreshes Monaco; next edit does not clobber` — simulates the Style Manager write path, verifies the Monaco buffer refreshes, then drives a real `onDidChangeModelContent` edit via `executeEdits` and asserts both rules survive in `globalCSS` (the prior coverage gap: the old Custom CSS spec deliberately bypassed Monaco). `Splitter drag in a windowed layout: sidebars can shrink, never force-grow` — first spec to drive GL splitters with real mouse events; note it must click the first-run welcome's dismiss button because the `.gstrap-modal-overlay` otherwise swallows all pointer input (evaluate()-driven specs never noticed).
