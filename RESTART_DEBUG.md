...
- Specs: 77/77 green via `xvfb-run -a npx playwright test` (~3.2m). Shared helpers in `tests/e2e/helpers.js` (launch, openSeedProject, selectFirstByTag, dismissWelcome, EXPECTED_PLUGIN_COUNT). Suite drives `dist/main/main.js` — `npm run build` REQUIRED before e2e after any src/ change.
...
**The campaign roadmap is `GRAPESTRAP_BUILD_PLAN_v5.md`** (v4 stays canonical for feature specs only). Next: Wave 2 — Master Templates (integrates first, failing round-trip spec is deliverable #1), then drag-to-resize. Known product decision parked for Wave 2: undo-across-code-view-sync has no contract (see sandbox-artifacts/grapestrap/w0-coverage-specs/PLAN.md). Read the auto-memory entry at `~/.claude/projects/-home-numb1/memory/session_2026_05_05_grapestrap_alpha9_12.md` for the full architecture context (still canonical for the layout notes below).
...
1. ~~**Properties↔Custom CSS source-of-truth sync**~~ — **FIXED 2026-07-06.** Root cause was NOT separate CSS scopes (this doc previously mis-described it): Properties panels (`style-manager/background.js`, `pseudo-class.js`) and Custom CSS both write the SAME `projectState.current.globalCSS`, but `custom-css/index.js` only re-read state into Monaco on `project:opened` — never on `project:css-changed` — so after a Properties write the Monaco buffer was stale and the next Custom CSS keystroke clobbered the Properties rule (last-writer-wins, one direction). Fix: guarded `project:css-changed` listener in `custom-css/index.js` (value-equality check makes the panel's own debounced emit a no-op; `refreshingFromState` flag keeps the programmatic `setValue` out of the change handler; cursor restored across refresh). `getCssEditor()` is now exposed on `window.__gstrap` as the spec handle. Regression spec: "Properties→Custom CSS sync" in `smoke.spec.js`.

2. ~~**Splitter snap-then-lock + crash mid-drag**~~ — **snap/lock FIXED 2026-07-06** (same session as #1). NOT an RO race: GL v2's `onSplitterDragStart` sums the `minWidth` of every tab in a stack (540px for our 3-tab side stacks); below that width the drag clamp forced every drag to jump the sidebar OUT to 540px and stick ("snaps then sticks" maximized; "only gets larger" windowed — second nola1 report 2026-07-06). Per-tab minWidth is now the per-stack floor ÷ 3 in `golden-layout-config.js`; spec drives real splitter mouse-drags (must dismiss the welcome overlay first — it eats pointer input in specs). The one-off `atom_cache.cc` crash was never reproduced; if it recurs it's a separate report.
